"""Code generated by Speakeasy (https://speakeasy.com). DO NOT EDIT."""

from __future__ import annotations
from .accountresponse import AccountResponse, AccountResponseTypedDict
from .integrationapplicationresponse import (
    IntegrationApplicationResponse,
    IntegrationApplicationResponseTypedDict,
)
from .userresponse import UserResponse, UserResponseTypedDict
from discord_sdk.types import (
    BaseModel,
    Nullable,
    OptionalNullable,
    UNSET,
    UNSET_SENTINEL,
)
from discord_sdk.utils import validate_const
import pydantic
from pydantic import model_serializer
from pydantic.functional_validators import AfterValidator
from typing import List, Literal
from typing_extensions import Annotated, NotRequired, TypedDict


class DiscordIntegrationResponseTypedDict(TypedDict):
    id: str
    application: IntegrationApplicationResponseTypedDict
    scopes: List[str]
    type: Literal["discord"]
    name: NotRequired[Nullable[str]]
    account: NotRequired[Nullable[AccountResponseTypedDict]]
    enabled: NotRequired[Nullable[bool]]
    user: NotRequired[Nullable[UserResponseTypedDict]]


class DiscordIntegrationResponse(BaseModel):
    id: str

    application: IntegrationApplicationResponse

    scopes: List[str]

    TYPE: Annotated[
        Annotated[Literal["discord"], AfterValidator(validate_const("discord"))],
        pydantic.Field(alias="type"),
    ] = "discord"

    name: OptionalNullable[str] = UNSET

    account: OptionalNullable[AccountResponse] = UNSET

    enabled: OptionalNullable[bool] = UNSET

    user: OptionalNullable[UserResponse] = UNSET

    @model_serializer(mode="wrap")
    def serialize_model(self, handler):
        optional_fields = ["name", "account", "enabled", "user"]
        nullable_fields = ["name", "account", "enabled", "user"]
        null_default_fields = []

        serialized = handler(self)

        m = {}

        for n, f in self.model_fields.items():
            k = f.alias or n
            val = serialized.get(k)
            serialized.pop(k, None)

            optional_nullable = k in optional_fields and k in nullable_fields
            is_set = (
                self.__pydantic_fields_set__.intersection({n})
                or k in null_default_fields
            )  # pylint: disable=no-member

            if val is not None and val != UNSET_SENTINEL:
                m[k] = val
            elif val != UNSET_SENTINEL and (
                not k in optional_fields or (optional_nullable and is_set)
            ):
                m[k] = val

        return m
